...
So how is evolution initiated?

Kull suggested that evolution is driven by the choices organisms make. 
If early reptiles chose to warm their young by covering them with their arms, 
their choice exerted selective pressure on other individuals to evolve genetically more efficient means to achieve this, thus driving the evolution of feathers and birds!

This chapter presents a complete project using simulation to evaluate Kull's proposal.
...
function giveBirth(turtle1, model)
	if turtle1.energy > model.birthEnergy && length(model.agents) < model.popMax
		...
		if rand() < model.muRate
			...
			model[first(model.agents)[1]].geneticRadius = min(20, (parentRadius * (1 + model.muExtent * (1 - 2 * rand()))))
		else
...
function demo()
	...
	lime(a) = a.color == :lime
	orange(a) = a.color == :orange
	adata = [(lime, count), (orange, count)]
	fig, p = abmexploration(model; (agent_step!)=agent_step!, params, adata, ac=cellcolor)
	fig
end
end
